...
import Text.PrettyPrint.HughesPJ hiding (brackets, braces)
...
pp_array [] = lbrack <> rbrack
pp_array xs = brackets $ vcat $ punctuate comma $ map pp_value xs
...
pp_object [] = lbrace <> rbrace
pp_object xs = braces $ vcat $ punctuate comma $ map pp_field xs
